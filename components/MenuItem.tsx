...
export function MenuItem({ name, description, price, priceLarge }: MenuItemProps) {
  return (
    <div className="flex justify-between items-start gap-2">
      <div className="pr-2 flex-1">
        <h3 className="font-bold text-sm sm:text-base leading-snug">{name}</h3>
        {description && (
      ...
      <div className="flex gap-2 sm:gap-4 font-semibold text-stone-600 whitespace-nowrap text-sm sm:text-base">
        ...
        {priceLarge && <span className="w-12 sm:w-16 text-right">{priceLarge}</span>}
      </div>
    </div>
  );
}